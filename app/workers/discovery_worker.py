...
def setup_worker_systems(sender, **kwargs):
    ...
    try:
        ...
        initialization_success = asyncio.run(_ai_manager.initialize_models(core_models))
        ...
        for model_name in core_models:
            try:
                model = _ai_manager.get_model_pipeline(model_name)
                if model:
                    loaded_models.append(model_name)
                    logger.info(f"[WORKER STARTUP] AI Model '{model_name}' verified ready")
            except Exception as e:
                logger.error(f"[WORKER STARTUP] AI Model '{model_name}' verification failed: {e}")

        logger.info(f"[WORKER STARTUP] Core AI Models ready: {len(loaded_models)}/{len(core_models)}")
...
def process_discovered_profile(self, username: str, source_type: str = 'user_search') -> Dict[str, Any]:
    ...
    try:
        ...
        result = asyncio.run(_process_profile())
